package tonfall.prefab.audio
{
	...
	public final class ContinuousLoop extends Processor
	{
		...
		private var _barPosition:Number;
		 ...
		 */
		public function ContinuousLoop( decoder: IAudioDecoder, samplesTotal: Number, barDuration: Number = 1.0 )
		{
			_decoder = decoder;
		...
		{
			const loopStart: Number = Math.floor( _barPosition / _barDuration ) * _barDuration; // LAST LOOP STARTING POSITION

			//-> MAP MUSICAL LOOP POSITION TO LOOP SAMPLE POSITION
			
			const to: Number = _barPosition + TimeConversion.numSamplesToBars( info.numSignals, bpm );
			
			const sampleStart: Number = ( _barPosition - loopStart ) / _barDuration * _samplesTotal;
			const sampleEnd: Number   = ( to - loopStart ) / _barDuration * _samplesTotal;
